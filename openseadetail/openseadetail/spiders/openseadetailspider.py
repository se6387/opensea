...
class OpenSeaDetailSpider(scrapy.Spider):
    ...
    def extractData(self, response):
        ...
        # div.sc-29427738-0.dVNeWL  # sc-29427738-0 dVNeWL
        for details in response.css('div.sc-29427738-0.dVNeWL'):
            openSeaDetailsItem['cati'] = details.css('a.sc-1f719d57-0.fKAlPV::text').extract()
    ...
    def startRequests(self):
        self.deleteIfFileExists()

        headers = {
            'User-Agent': 'Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.1 (KHTML, like Gecko) Chrome/22.0.1207.1 Safari/537.1'
        }
        for url in self.start_urls:
            yield re(url, headers = headers)
